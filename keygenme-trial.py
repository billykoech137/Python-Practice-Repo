...
n = hashlib.sha256(bUsername_trial).hexdigest()
the_key = key_part_static1_trial + str(n[4]) + str(n[5]) + str(n[3]) + str(n[6]) + str(n[2]) + str(n[7]) + str(n[1]) + str(n[8]) + key_part_static2_trial
...
def check_key(key, username_trial):
    ...
    if len(key) != len(key_full_template_trial):
        ...
        print(key_part_static1_trial, len(key_part_static1_trial))
...
def decrypt_full_version(key_str):
    ...
    try:
    	...
    	SYSTEM_ERROR.stderr.write("Full version of keygenme NOT written to disk, "+ \
